#!/usr/bin/env python3
"""Generate scripts/entries/pN_chNN.py from OCR draft JSONL (bootstrap for curation)."""
...
def write_module(part: int, chapter: int, records: list[dict], topic_slug: str) -> Path:
    ...
    out = ENTRIES / f"p{part}_ch{chapter:02d}.py"
    lines = [
        f'"""Part {part}, Chapter {chapter} entries — OCR draft (needs curation)."""',
        "",
        "from __future__ import annotations",
        "",
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Bootstrap entry module from OCR JSONL")
    parser.add_argument("--part", type=int, required=True)
    parser.add_argument("--chapter", type=int, required=True)
    args = parser.parse_args()

    prefix = f"_ch{args.chapter:02d}" if args.part == 1 else f"_p{args.part}_ch{args.chapter:02d}"
    jsonl_path = DRAFTS / f"{prefix}_records.jsonl"
    topics = json.loads((DATA / "chapter_topics.json").read_text(encoding="utf-8"))
    key = f"RN-P{args.part}-C{args.chapter:02d}"
    slug = topics[key][0]
